    ...
    <div className="space-y-6">
      {warnings.length > 0 && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
          <h4 className="text-sm font-semibold text-amber-800">Evidence warnings</h4>
        ...
        <table className="w-full text-left text-sm">
          <thead className="border-b border-slate-100 bg-slate-50">
            <tr>
              ...
              <th className="hidden px-4 py-3 font-medium text-slate-600 md:table-cell">Type</th>
              <th className="hidden px-4 py-3 font-medium text-slate-600 lg:table-cell">Product</th>
              <th className="hidden px-4 py-3 font-medium text-slate-600 sm:table-cell">Supplier</th>
              <th className="px-4 py-3 font-medium text-slate-600">Status</th>
              <th className="hidden px-4 py-3 font-medium text-slate-600 md:table-cell">Visibility</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-50">
            {documents.map((doc) => (
              <tr key={doc.id} className="hover:bg-slate-50/50">
                ...
                <td className="hidden px-4 py-3 capitalize text-slate-600 md:table-cell">
                  {doc.type.replace(/_/g, " ")}
                </td>
                <td className="hidden px-4 py-3 text-slate-600 lg:table-cell">
                  {productMap[doc.linkedProductId] ?? "—"}
                </td>
                <td className="hidden px-4 py-3 text-slate-600 sm:table-cell">{doc.supplier}</td>
                <td className="px-4 py-3">
